...
namespace Eu.Iamia.Invoicing.Application.UnitTests.Configuration;
public class Setup : IHandlerSetup
{
    ...
    {
        var serviceDescriptor = services.FirstOrDefault(descriptor => descriptor.ServiceType == typeof(IRestApiGateway));
        if (serviceDescriptor is null)
        {
            throw new ArgumentException($"Unable to remove standard service for {nameof(IRestApiGateway)} before adding {nameof(RestApiGatewayStub)}");
        }
        services.Remove(serviceDescriptor);
        services.AddTransient<IRestApiGateway, RestApiGatewayStub>();
    }
