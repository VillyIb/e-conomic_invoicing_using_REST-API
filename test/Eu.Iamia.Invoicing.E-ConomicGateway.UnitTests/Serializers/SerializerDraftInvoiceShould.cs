...
namespace Eu.Iamia.Invoicing.E_ConomicGateway.UnitTests.Serializers;

[NCrunch.Framework.Category("Unit")]
public class SerializerDraftInvoiceShould
{
    ...
    private readonly SerializerDraftInvoice _sut = new(new JsonSerializerFacade());

    [Fact]
    public void Deserialize_When_ValidJson_Expect_DraftInvoice()
    {
        var invoice = _sut.Deserialize(DraftInvoiceJson);
    ...
    [Fact]
    public void Deserialize_When_InvalidJson_Throws_JsonException()
    {
        var invalidJson = DraftInvoiceJson.Replace("{", "[");
        _ = Assert.Throws<JsonException>(() => _sut.Deserialize(invalidJson));
    }
}